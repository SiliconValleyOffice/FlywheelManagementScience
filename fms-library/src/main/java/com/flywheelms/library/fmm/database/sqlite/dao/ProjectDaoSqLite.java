...
public class ProjectDaoSqLite extends CompletableNodeDaoSqLite<Project> {

	private static ProjectDaoSqLite singleton;

	public static ProjectDaoSqLite getInstance() {
		if(ProjectDaoSqLite.singleton == null) {
			ProjectDaoSqLite.singleton = new ProjectDaoSqLite();
		}
		return ProjectDaoSqLite.singleton;
	}
	
	@Override
	public FmmNodeDefinition getFmmNodeDefinition() {
		return FmmNodeDefinition.PROJECT;
	}
    ...
    @Override
    public ContentValues buildUpdateContentValues(Project aProject) {
        return buildContentValues(aProject);
    }

	@Override
	protected Project getNextObjectFromCursor(Cursor aCursor) {
		Project theProject = new Project(aCursor.getString(this.columnIndexMap.get(IdNodeMetaData.column_ID)) );
		getColumnValues(this.columnIndexMap, aCursor, theProject);
		return theProject;
	}
