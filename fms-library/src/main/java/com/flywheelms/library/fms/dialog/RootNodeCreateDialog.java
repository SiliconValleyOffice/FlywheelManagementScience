...
public abstract class RootNodeCreateDialog extends FmsCancelOkApplyFdkDialog {
	...
	public RootNodeCreateDialog(GcgActivity aLibraryActivity, FmsTreeViewAdapter aTreeViewAdapter, FmmNodeDefinition anFmmNodeDefinition) {
    ...
    protected void initializeDialogBodyLate() {
        ...
        this.headlineWidget.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                RootNodeCreateDialog.this.manageButtonState();
            }
        });
    ...
    protected void onClickButtonOk() {
        FmmHeadlineNode theFmmHeadlineNode = createRootNode();
        if(theFmmHeadlineNode != null) {
            this.treeViewAdapter = this.treeViewAdapter.addNewHeadlineNode(theFmmHeadlineNode);
            GcgHelper.makeToast(this.fmmNodeTypeWidget.getText() + " created: " + this.fmmNodeTypeWidget.getText());
            if(this.editNewFmmNode.isChecked()) {
                this.treeViewAdapter.editTreeNodeObject(theFmmHeadlineNode);
            }
        } else {
            GcgHelper.makeToast("ERROR:  Unable to create " + this.fmmNodeTypeWidget.getText());
        }
        this.gcgActivity.stopDialog();
    }

    protected abstract FmmHeadlineNode createRootNode();
