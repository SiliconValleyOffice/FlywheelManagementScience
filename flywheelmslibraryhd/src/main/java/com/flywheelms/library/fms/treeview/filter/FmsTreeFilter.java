...
public class FmsTreeFilter extends GcgTreeFilter {
	...
	public FmsTreeFilter(View aTreeView) {
		super(aTreeView);
	}
	
	private FmsTreeViewParent getFmsTreeViewParent() {
		return (FmsTreeViewParent) this.treeViewParent;
	}

	@Override
	public boolean showInTree(GcgTreeNode aTreeNodeInfo) {
		boolean theBoolean = getFmsTreeViewParent().isShowAllNodeCompletions() || aTreeNodeInfo.getLevel() == -1;
		if(theBoolean) {
			return true;
