/* @(#)SequencedLinkNodeDaoSqLite.java
** 
...
import com.flywheelms.library.fmm.node.impl.link.FmmSequencedLinkNodeImpl;

public abstract class SequencedLinkNodeDaoSqLite<T extends FmmSequencedLinkNodeImpl> extends LinkNodeDaoSqLite<T> {
	...
	protected void getColumnValues(HashMap<String, Integer> aHashMap, Cursor aCursor, T anFmmNode) {
		...
		anFmmNode.setSequence(aCursor.getInt(aHashMap.get(SequencedLinkNodeMetaData.column_SEQUENCE)));
	}

	@Override
	public ContentValues buildContentValues(T anFmmNode) {
		ContentValues theContentValues = super.buildContentValues(anFmmNode);
		theContentValues.put(SequencedLinkNodeMetaData.column_SEQUENCE, anFmmNode.getSequence());
